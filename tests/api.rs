...
use flacx::{EncodeOptions, FlacEncoder, encode_bytes, encode_file, level::Level};
...
fn top_level_encode_bytes_matches_default_encoder() {
    ...
    let via_encoder = FlacEncoder::default().encode_bytes(&wav).unwrap();
...
fn convenience_encode_file_matches_default_encoder_output() {
    ...
    let bytes_from_memory = FlacEncoder::default().encode_bytes(&wav).unwrap();
